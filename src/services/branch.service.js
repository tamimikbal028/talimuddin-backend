...
const removeMemberService = async (
  ...
  if (branch.creator_id === targetUserId) {
...
const getBranchMembersService = async (branchId, userId, queryParams) => {
  ...
  const {
    ...
    .select(
      ...
      is_admin,
      created_at,
      user:users!user_id(id, full_name, user_name, avatar)
    `,
      { count: "exact" }
    )
    .eq("branch_id", branchId)
  ...
  const members = (memberships || [])
    .map((membership) => {
      ...
      if (!u) return null; // Handle orphaned membership

      const isSelf = u.id === userId;
      const targetIsCreator = branch.creator_id === u.id;
      const targetIsAdmin = membership.is_admin;

      // Creator can manage anyone except self; Admin can manage regular members
      const canManage =
        !isSelf &&
        (isCreator || (isAdmin && !targetIsAdmin && !targetIsCreator));
